// src/app/admin/reto/winners/route.ts
...
async function requireAdmin(req: NextRequest) {
  // Auth: sesión Supabase vía cookies (server-side) + email admin
  try {
    ...
    const supabase = createServerClient(url, anon, {
      cookies: {
        ...
        // ✅ Capturamos cookies para pegarlas en la respuesta
        setAll(list) {
    ...
    if (error || !email) return { ok: false as const, cookiesToSet };
    if (email !== adminEmail) return { ok: false as const, cookiesToSet };
...
// GET /admin/reto/winners?group=GRUPOA&status=pendiente
...
// PATCH /admin/reto/winners body: { id, status }
