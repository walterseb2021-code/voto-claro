...
type Filter = 'pending' | 'active' | 'disqualified';
...
export default function AdminProyectosPage() {
  ...
  const loadProjects = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/proyectos?status=${encodeURIComponent(filter)}`,
        {
          method: 'GET',
          credentials: 'include',
          cache: 'no-store',
      ...
      console.error('Error cargando proyectos admin:', err);
  ...
  const updateStatus = async (
    ...
    try {
      ...
      if (
        response.status === 401 ||
        (response.status === 403 && result?.error !== 'origin_invalid')
      ) {
        redirectToAdminLogin();
        return;
      }
      ...
      setMessage(
        `✅ Proyecto ${newStatus === 'active' ? 'aprobado' : 'rechazado'} correctamente`
      );
      await loadProjects();
    } catch {
  ...
  const evaluateProject = async (project: Project) => {
    const minSupportsRequired = getMinimumSupports(
      project.minimum_supports_required
    );
    const currentSupports = getSupportCount(project.beneficiary_count);

    if (minSupportsRequired == null) {
      alert('Este proyecto no tiene un umbral de apoyos válido configurado.');
      return;
    }

    if (currentSupports < minSupportsRequired) {
      alert(
        `Este proyecto todavía no puede evaluarse. Le faltan ${
          minSupportsRequired - currentSupports
        } apoyos.`
      );
      return;
    }
    ...
    const clarity = askScore(
      'Puntaje de claridad del problema y la solución',
      15
    );
    if (clarity == null) return;

    const viability = askScore(
      'Puntaje de viabilidad técnica y presupuestal',
      15
    );
    if (viability == null) return;

    const sustainability = askScore(
      'Puntaje de sostenibilidad del beneficio',
      15
    );
    ...
    try {
      const response = await fetch('/api/admin/proyectos/evaluar', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        cache: 'no-store',
      ...
      if (
        response.status === 401 ||
        (response.status === 403 && data?.error !== 'origin_invalid')
      ) {
        redirectToAdminLogin();
        return;
      }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-2xl border-2 border-emerald-600 p-4 mb-6 shadow-sm">
          ...
          <div className="text-sm text-slate-700 space-y-1">
            <p>
              Cada proyecto se evalúa con el <strong>umbral de apoyos registrado en su propia postulación</strong>. Los proyectos históricos pueden conservar umbrales distintos a los de convocatorias posteriores.
            </p>
            <p>
              La nota final combina <strong>40 puntos por respaldo ciudadano</strong> y <strong>60 puntos por calidad del proyecto</strong>.
            </p>
            <p>
              La calidad se califica con cuatro criterios de hasta <strong>15 puntos</strong> cada uno: impacto comunitario, claridad del problema y la solución, viabilidad técnica y presupuestal, y sostenibilidad del beneficio.
            </p>
          </div>
        </div>
            ...
            {projects.map((project) => {
              ...
              const supportsRemaining =
                minSupportsRequired == null
                  ? null
                  : Math.max(minSupportsRequired - currentSupports, 0);
                  ...
                  <div className="flex justify-between items-start flex-wrap gap-3">
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-slate-500">
                        ...
                        {project.leader?.full_name || 'Anónimo'}
                        {project.leader?.alias
                          ? ` (@${project.leader.alias})`
                          : ''}
                      </div>
                      <div className="text-xs text-slate-500">
                      ...
                      <div className="text-xs text-slate-500">
                        🤝 Apoyos: {currentSupports} /{' '}
                        {minSupportsRequired ?? 'No configurado'}
                      </div>
                      <div className="text-xs text-slate-500">
                        💰 Monto solicitado:{' '}
                      ...
                      {filter === 'active' ? (
                        <>
                          {project.evaluation_exists ? (
                            ...
                            <span className="text-xs text-red-700 bg-red-50 px-3 py-2 rounded-xl">
                              Umbral no configurado
                            </span>
                        ...
                        <>
                          ...
                          <button
                            onClick={() =>
                              updateStatus(project.id, 'disqualified')
                            }
