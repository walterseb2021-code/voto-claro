-- ============================================================
-- B2C-E5F6A
-- EXTENDER RESULTADO AUTORITATIVO DE RESPUESTA ATOMICA DEL RETO
-- ============================================================
--
-- Objetivo:
--   Mantener toda la decision y persistencia de la respuesta dentro de
--   PostgreSQL y devolver, desde la misma transaccion, el state final y
--   finished_at. Asi /secure/answer no necesita una lectura posterior al
--   COMMIT para construir la respuesta HTTP.
--
...
begin
  ...
  if v_proc is null then
    raise exception 'B2C_E5F6A_ABORT: RPC base no existe';
  end if;
  ...
  if v_owner <> 'postgres' then
    raise exception 'B2C_E5F6A_ABORT: owner inesperado %', v_owner;
  end if;

  if not v_security_definer then
    raise exception 'B2C_E5F6A_ABORT: SECURITY DEFINER ausente';
  end if;

  if coalesce(v_search_path, '') <> 'search_path=pg_catalog' then
    raise exception 'B2C_E5F6A_ABORT: search_path inesperado %', v_search_path;
  end if;

  if v_result not ilike '%award_quarter smallint%'
     or v_result ilike '%session_state jsonb%'
     or v_result ilike '%finished_at timestamp with time zone%' then
    raise exception 'B2C_E5F6A_ABORT: firma de retorno base inesperada %', v_result;
  end if;
  ...
  if pg_catalog.has_function_privilege('anon', v_proc, 'EXECUTE')
    ...
    raise exception 'B2C_E5F6A_ABORT: ACL de ejecucion inesperada';
  end if;
  ...
  if pg_catalog.has_table_privilege(
    ...
    raise exception 'B2C_E5F6A_ABORT: ACL inesperada en instancias privadas';
  end if;
end
$preflight$;
...
drop function public.commit_reto_prize_answer_atomic(
...
begin
  ...
  if not found or v_return_state is null then
    raise exception 'RETO_ANSWER_RESULT_STATE_INVALID';
  end if;
...
begin
  ...
  if v_proc is null then
    raise exception 'B2C_E5F6A_POSTFLIGHT: RPC no existe';
  end if;
  ...
  if v_owner <> 'postgres' then
    raise exception 'B2C_E5F6A_POSTFLIGHT: owner inesperado %', v_owner;
  end if;

  if not v_security_definer then
    raise exception 'B2C_E5F6A_POSTFLIGHT: SECURITY DEFINER ausente';
  end if;

  if coalesce(v_search_path, '') <> 'search_path=pg_catalog' then
    raise exception 'B2C_E5F6A_POSTFLIGHT: search_path inesperado %', v_search_path;
  end if;

  if v_result not ilike '%session_state jsonb%'
     or v_result not ilike '%finished_at timestamp with time zone%' then
    raise exception 'B2C_E5F6A_POSTFLIGHT: retorno extendido ausente %', v_result;
  end if;

  if pg_catalog.has_function_privilege('anon', v_proc, 'EXECUTE')
     or pg_catalog.has_function_privilege('authenticated', v_proc, 'EXECUTE')
     or not pg_catalog.has_function_privilege('service_role', v_proc, 'EXECUTE') then
    raise exception 'B2C_E5F6A_POSTFLIGHT: ACL de ejecucion inesperada';
  end if;
  ...
  if pg_catalog.has_table_privilege(
    ...
    raise exception 'B2C_E5F6A_POSTFLIGHT: ACL inesperada en instancias privadas';
  end if;
end
$postflight$;